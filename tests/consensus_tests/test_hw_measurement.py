...
def test_measuring_hw_for_updates_without_waiting(tmp_path: pathlib.Path):
    ...
    for peer_idx in range(N_PEERS):
        ...
        # Assert that each nodes telemetry has been increased by some bytes
        assert peer_hw["payload_io_write"] >= (approx_points_on_node * 5)  # ~50 (45) vectors on this node with payload of ~5 bytes
        assert vector_writes >= (approx_points_on_node * 4 * 4)  # ~50 (45) vectors on this node with 4 dim and 4 bytes

    # Ensure all vectors have been accounted for
    assert total_vectors_metrics >= total_vectors * 4 * 4
