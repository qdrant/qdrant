...
def upsert_random_points(peer_url, num, collection_name="test_collection", fail_on_error=True, offset=0, wait='true'):
    # Create points in first peer's collection
    r_batch = requests.put(
        f"{peer_url}/collections/{collection_name}/points?wait={wait}", json={
            "points": [
                {
                    "id": i + offset,
