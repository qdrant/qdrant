#!/bin/bash
# This test checks that Read-Write API keys allow all operations
...
(($docker_grpcurl -H 'api-key: my-secret' -d '{
...
(($docker_grpcurl -H 'api-key: my-secret' -d '{}' $QDRANT_HOST qdrant.Collections/List &> /dev/null
) && echo "Succeeded") || echo "Failed"
...
(($docker_grpcurl -H 'api-key: my-secret' -d '{ "collection_name": "test_collection" }' $QDRANT_HOST qdrant.Collections/Get &> /dev/null
) && echo "Succeeded") || echo "Failed"
