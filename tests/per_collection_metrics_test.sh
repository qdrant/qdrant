...
curl -s -X PUT "$QDRANT_HOST/collections/$TEST_COLLECTION"   -H "Content-Type: application/json"   -d '{"vectors": {"size": 4, "distance": "Cosine"}}' > /dev/null

echo "Inserting test point..."
curl -s -X PUT "$QDRANT_HOST/collections/$TEST_COLLECTION/points"   -H "Content-Type: application/json"   -d '{"points":[{"id":1,"vector":[0.1,0.2,0.3,0.4],"payload":{"test":"data"}}]}' > /dev/null
...
echo "Cleaning up..."
curl -s -X DELETE "$QDRANT_HOST/collections/$TEST_COLLECTION" > /dev/null
