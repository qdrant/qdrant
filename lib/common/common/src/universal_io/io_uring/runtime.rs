...
pub struct IoUringState<'data, U> {
    requests: Slab<(U, IoUringRequest<'data>)>,
}
    ...
    U: UserData,
{
    ...
    fn read_exact(&mut self, user_data: U, fd: Fd, range: Range<u64>, align: usize) -> squeue::Entry {
        let size =
            u32::try_from(range.end - range.start).expect("read buffer length fit within u32");
        let buffer = AlignedBuf::with_capacity(range.start, align, size as usize);
    ...
    pub fn finalize(
        ...
        let resp = match req {
            ...
            IoUringRequest::ODirectRead {
                ...
                let mut result = AlignedBuf::new_for_offset(inner_range.start as u64, align);
                result.extend_from_slice(&buffer[inner_range]);
...
impl<'data> IoUringRequest<'data> {
    pub fn expect_read(&mut self) -> &mut AlignedBuf {
        match self {
...
pub enum IoUringResponse {
    /// An [`AlignedBuf`] containing exactly the bytes the caller asked for,
    /// satisfying the alignment invariant for the read's `file_offset`.
    Read(AlignedBuf),
    Write,
}

impl IoUringResponse {
    pub fn expect_read(self) -> AlignedBuf {
