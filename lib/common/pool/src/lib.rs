...
struct PoolTasks<GroupId> {
    ...
    ready_to_run_tasks: BinaryHeap<RevQueuePair<TaskId, ReadyToRunTask<GroupId>>>,
    waiting_tasks: HashMap<GroupId, TaskGroup>,
}
...
impl TaskGroup {
    ...
    fn task_is_complete<GroupId: Clone>(
        &mut self,
        complete_task_operation_mode: OperationMode,
        group_id: &GroupId,
        ready_to_run_tasks: &mut BinaryHeap<RevQueuePair<TaskId, ReadyToRunTask<GroupId>>>,
    ) {
    ...
    fn refill_ready_to_run_tasks<GroupId: Clone>(
        ...
        while let Some((mode, task_id, task)) = self.try_get_next_runnable_task() {
            let ready_task = ReadyToRunTask::<GroupId> {
                group_id: group_id.clone(),
                task,
                mode,
            };
            ready_to_run_tasks.push(RevQueuePair::new(task_id, ready_task));
        }
    }
}
...
impl<Id: Clone + Eq + Hash> Pool<Id> {
    pub fn submit(&self, group_id: Id, mode: OperationMode, task: Task) {
        ...
        let task_id = guard.next_available_priority;
        guard.next_available_priority += 1;
...
fn thread_worker<Id: Eq + Hash + Clone>(
    ...
    loop {
        let ready_to_run_task = {
            let mut guard = tasks.lock();
            loop {
                if let Some(pair) = guard.ready_to_run_tasks.pop() {
                    let ready_to_run_task = pair.1;
        ...
        let _result = catch_unwind(ready_to_run_task.task);
...

        {
            ...
            let group = waiting_tasks
                .get_mut(&ready_to_run_task.group_id)
                .expect("missing task group");
            group.task_is_complete(
                ready_to_run_task.mode,
                &ready_to_run_task.group_id,
                // refill ready to run tasks if possible
                ready_to_run_tasks,
            );
            if group.is_empty() {
                // Includes both running, queued and waiting tasks.
                waiting_tasks.remove(&ready_to_run_task.group_id);
            }
        }
    }
}
