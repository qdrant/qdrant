...
/// pipeline over its reply channel. The slot is the correlation id; the bytes
/// have already been written into the pipeline-owned destination buffer, so
/// `result` carries only the completion status.
#[derive(Debug)]
...
impl BridgeResponse {
    /// Build a reply for the given slot with the future's completion status.
    /// Provided so the spawned task doesn't have to reach into the struct
    /// layout when constructing the response.
    pub(crate) fn new(slot: u64, result: Result<(), UniversalIoError>) -> Self {
        Self { slot, result }
    }
}
