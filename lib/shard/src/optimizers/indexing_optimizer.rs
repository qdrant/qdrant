...
impl IndexingOptimizer {
    ...
    /// ever reduces the segment count, and the result lands past the larger
    /// of the full-scan boundary and the indexing threshold, so this
    /// converges.
    fn plan_sub_full_scan_tail_merges(&self, planner: &mut OptimizationPlanner) {
        ...
        let mut batch = tails
            ...
            .collect_vec();
        // The segment optimizer rebuilds the batch with an HNSW index only
        // when the largest vector field of the result reaches
        // `indexing_threshold_kb`. A result above the full-scan boundary but
        // below the indexing threshold would be rebuilt `Plain` — still
        // scanned in full and possibly dropping an index an input segment
        // already had — so the crossing check below uses the larger of the
        // two thresholds per field.
        let indexing_threshold_bytes = self
            .thresholds_config
            .indexing_threshold_kb
            .saturating_mul(BYTES_IN_KB);
        ...
        let crosses = |field_sums: &[usize]| {
            field_sums
                ...
                .any(|(&sum, &(_, threshold))| sum >= threshold.max(indexing_threshold_bytes))
        };
        // Merge the tails into the smallest regular segment that fits. With
        // a single vector field a regular segment already sits above the
        // full-scan boundary by itself, but the batch as a whole still has
        // to cross the effective threshold checked below.
        ...
        // Plan only when some field of the merged result actually crosses
        // the larger of its full-scan boundary and the indexing threshold
        // (with a partner that holds by construction; tails alone must add
        // up to it). Merging keeps the scanned point count the same, so a
        // batch that stays below would reduce the segment count — possibly
        // far below the target — without making anything searchable by
        // graph. In particular, when `full_scan_threshold` exceeds
