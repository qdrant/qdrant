...
/// - Each shard is a fully independent S3-FIFO cache
    ...
    S: BuildHasher + Default + Clone,
{
    ...
    /// - `num_shards`: Number of shards (power of 2 recommended for best performance)
    ...
    /// - `hasher`: Custom hasher for key hashing
    ///
    /// # Panics
    ///
    /// Panics if `num_shards` is zero or if `capacity` is less than `num_shards`.
    pub fn with_config(
        capacity: usize,
    ...
    fn get_shard_by_hash(&self, hash: u64) -> &Cache<K, V, L, S> {
        ...
        // SAFETY: idx is always valid because shard_selector returns indices within bounds
