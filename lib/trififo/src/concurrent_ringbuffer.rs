...
//! - Only one writer thread performing mutations
...
/// A ring buffer that supports lock-free reads with a single writer.
///
/// Writers must be externally synchronized (e.g., via a channel to a dedicated thread).
/// Readers can access entries concurrently by absolute offset.
...
// Safety: ConcurrentRingBuffer can be shared between threads because:
// - Reads are done via UnsafeCell with proper synchronization
// - Writes are serialized externally (single writer thread)
// - Atomic operations provide necessary memory ordering
unsafe impl<T: Send> Send for ConcurrentRingBuffer<T> {}
unsafe impl<T: Send + Sync> Sync for ConcurrentRingBuffer<T> {}
...
impl<T> ConcurrentRingBuffer<T> {
    ...
    // ========================================================================
    // Writer-only methods below. These must only be called from a single thread.
    // ========================================================================
    ...
    pub fn overwriting_push(&self, item: T) -> usize {
        let offset = self.write_pos.load(Ordering::Relaxed);

        // Write the new value
        // Safety: Single writer ensures no concurrent writes to same offset
        unsafe {
    ...
    pub fn reinsert_unchecked_if(&self, f: impl FnOnce(&T) -> bool) -> bool {
...
mod tests {
    ...
    fn test_new_buffer_is_empty() {
        let buffer: ConcurrentRingBuffer<i32> = ConcurrentRingBuffer::with_capacity(5);
    ...
    fn test_zero_capacity_panics() {
        let _buffer: ConcurrentRingBuffer<i32> = ConcurrentRingBuffer::with_capacity(0);
    }

    #[test]
    fn test_push_and_get() {
        let buffer = ConcurrentRingBuffer::with_capacity(3);
    ...
    fn test_pop_push_unchecked() {
        ...
        let (evicted, new_offset) = buffer.pop_push_unchecked(40);
        assert_eq!(evicted, 10);
        assert_eq!(new_offset, 0);
        assert_eq!(buffer.get_absolute_unchecked(new_offset), &40);
    }
}