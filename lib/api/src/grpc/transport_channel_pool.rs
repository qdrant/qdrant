...
use super::tls::WithClientMutualTlsCertificates;
...
impl ChannelPool {
    async fn init(
        ...
        for _ in 0..pool_size.into() {
            let channel =
                TransportChannelPool::make_channel(grpc_timeout, connection_timeout, uri.clone(), &tls_certificates)
                    .await?;
            channels.push(channel);
        }
        let fast_channel =
            TransportChannelPool::make_channel(SMART_CONNECT_TIMEOUT, connection_timeout, uri, &tls_certificates)
                .await?;
...
impl TransportChannelPool {
    ...
    pub async fn make_channel(
        ...
        let endpoint = Channel::builder(uri)
            .with_client_mutual_tls_certificates(&tls_certificates)
    ...
    async fn init_pool_for_uri(&self, uri: Uri, tls_certificates: &Option<ClientMutualTlsCertificates>) -> Result<Channel, TonicError> {
    ...
    async fn get_or_create_pooled_channel(&self, uri: &Uri, tls_certificates: &Option<ClientMutualTlsCertificates>) -> Result<Channel, TonicError> {
        match self.get_pooled_channel(uri).await {
            None => self.init_pool_for_uri(uri.clone(), &tls_certificates).await,
    ...
    pub async fn with_channel_timeout<T, O: Future<Output = Result<T, Status>>>(
        ...
        let channel = self.get_or_create_pooled_channel(uri, &tls_certificates).await?;
    ...
    pub async fn with_channel<T, O: Future<Output = Result<T, Status>>>(
        ...
        self.with_channel_timeout(uri, &tls_certificates, f, None).await
    }
}

#[derive(Debug, Clone)]
pub struct ClientMutualTlsCertificates {
    pub client_certificate: String,
    pub client_certificate_key: String,
