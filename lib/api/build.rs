...
fn configure_validation(builder: Builder) -> Builder {
    builder
        ...
        .validates(&[
            ...
            ("NearestInputWithFocus.nearest", ""),
            ("NearestInputWithFocus.focus", ""),
            ("DimsFocus.dims", "length(min = 1)"),
            ("DimsFocus.candidates_limit", "range(max = 16_384)"),
            ("WithDimsExplained.top", "range(min = 1, max = 65_536)"),
            ("Rrf.k", "range(min = 1)"),
            ("Query.variant", ""),
