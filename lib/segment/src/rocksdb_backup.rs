...
pub fn create(db: &rocksdb::DB, backup_path: &Path) -> OperationResult<()> {
    ...
    backup_engine(backup_path)?.create_new_backup(db).map_err(
        |err| OperationError::service_error(format!("failed to create RocksDB backup: {err}"))
    )
}
...
pub fn restore(backup_path: &Path, restore_path: &Path) -> OperationResult<()> {
    backup_engine(backup_path)?
        ...
        .map_err(
            |err| OperationError::service_error(format!("failed to restore RocksDB backup: {err}"))
        )
}
...
fn backup_engine(path: &Path) -> OperationResult<rocksdb::backup::BackupEngine> {
    let options = rocksdb::backup::BackupEngineOptions::new(path).map_err(|err| {
        OperationError::service_error(
            format!("failed to create RocksDB backup engine options: {err}")
        )
    })?;
    let env = rocksdb::Env::new().map_err(|err| {
        OperationError::service_error(
            format!("failed to create RocksDB backup engine environment: {err}")
        )
    })?;
    rocksdb::backup::BackupEngine::open(&options, &env).map_err(|err| {
        OperationError::service_error(
            format!("failed to open RocksDB backup engine {path:?}: {err}")
        )
    })
}
...
fn create_dir_all(path: &Path) -> OperationResult<()> {
    fs::create_dir_all(path).map_err(|err| {
        OperationError::service_error(
            format!("failed to create RocksDB backup directory {path:?}: {err}")
        )
    })
}
...
fn directory_not_empty_error(path: &Path) -> OperationError {
    OperationError::service_error(
        format!("RockDB backup directory {path:?} already exists and is not empty")
    )
}
