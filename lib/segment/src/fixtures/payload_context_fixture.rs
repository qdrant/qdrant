...
use crate::fixtures::payload_fixtures::{FLT_KEY, generate_diverse_payload, GEO_KEY, INT_KEY, STR_KEY};
...
use crate::payload_storage::in_memory_payload_storage::{InMemoryConditionChecker, InMemoryPayloadStorage};
...
impl IdTracker for IdsIterator {
    ...
    fn set_version(&mut self, _external_id: PointIdType, _version: SeqNumberType) -> OperationResult<()> {
        Ok(())
    }
    ...
    fn set_link(&mut self, _external_id: PointIdType, _internal_id: PointOffsetType) -> OperationResult<()> {
        Ok(())
    }
    ...
    fn iter_external(&self) -> Box<dyn Iterator<Item=PointIdType> + '_> {
        Box::new(self.ids.iter().copied().map(|id| PointIdType::NumId(id as u64)))
    }
    ...
    fn iter_from(&self, external_id: Option<PointIdType>) -> Box<dyn Iterator<Item=(PointIdType, PointOffsetType)> + '_> {
...
fn create_payload_storage(num_points: usize) -> InMemoryPayloadStorage {
    ...
    for id in 0..num_points {
        ...
        payload_storage.assign(id as PointOffsetType, &payload).unwrap();
    }
...
pub fn create_plain_payload_index(path: &Path, num_points: usize) -> PlainPayloadIndex {
    ...
    let condition_checker = Arc::new(
        InMemoryConditionChecker::new(
            Arc::new(AtomicRefCell::new(payload_storage)),
            ids_iterator.clone(),
        )
    );

    PlainPayloadIndex::open(
        condition_checker, ids_iterator, path,
    ).unwrap()
}
...
pub fn create_struct_payload_index(path: &Path, num_points: usize) -> StructPayloadIndex {
    ...
    index.set_indexed(STR_KEY, PayloadSchemaType::Keyword).unwrap();
    index.set_indexed(INT_KEY, PayloadSchemaType::Integer).unwrap();
    index.set_indexed(FLT_KEY, PayloadSchemaType::Float).unwrap();
