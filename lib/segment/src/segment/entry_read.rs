...
use crate::common::{check_query_vectors, check_stopped, check_vector_name};
...
use crate::entry::read_entry_point::SegmentEntryRead;
...
use crate::types::{
    Filter, Payload, PayloadFieldSchema, PayloadIndexInfo, PayloadKeyType, PointIdType,
...
impl SegmentEntryRead for Segment {
    ...
    fn all_vectors(
        ...
        for vector_name in self.vector_data.keys() {
            if let Some(vec) = SegmentEntryRead::vector(self, vector_name, point_id, hw_counter)? {
    ...
    fn estimate_point_count<'a>(
        ...
        match filter {
            None => {
                let available = SegmentEntryRead::available_point_count(self);
                CardinalityEstimation {
                    primary_clauses: vec![],
                    min: available,
                    exp: available,
    ...
    fn size_info(&self) -> SegmentInfo {
        ...
        let num_points = SegmentEntryRead::available_point_count(self);
        ...
        SegmentInfo {
            ...
            num_vectors,
            num_indexed_vectors,
            num_points: SegmentEntryRead::available_point_count(self),
            num_deleted_vectors: SegmentEntryRead::deleted_point_count(self),
    ...
    fn info(&self) -> SegmentInfo {
        ...
        let mut info = SegmentEntryRead::size_info(self);
    ...
    fn get_telemetry_data(&self, detail: TelemetryDetail) -> SegmentTelemetry {
        ...
        SegmentTelemetry {
            info: SegmentEntryRead::info(self),
            config: SegmentEntryRead::config(self).clone(),
    ...
    fn fill_query_context(&self, query_context: &mut QueryContext) {
        query_context.add_available_point_count(SegmentEntryRead::available_point_count(self));
