...
/// with a nested `min_should`. Dropping one shape would exclude every point
/// that only carries a value of that shape from the counting pass.
fn candidate_match_filter(key: &JsonPath, candidates: &HashSet<FacetValue>) -> Filter {
    ...
    match (strings.is_empty(), integers.is_empty()) {
        ...
        // Nest the OR in a single `must` condition: `merge_owned` flattens
        // `min_should` lists (union of conditions, max of min_counts), so a
        // top-level `min_should` here would lose its grouping when merged
        // with a request filter that has its own `min_should`, making the
        // candidate constraint optional instead of required.
        (false, false) => Filter::new_must(Condition::Filter(Filter::new_min_should(
            MinShould {
                conditions: vec![
...
mod tests {
    ...
    /// The candidate OR must survive merging with a request filter that has
    /// its own `min_should`: the merged filter still requires one candidate,
    /// not any single condition from either side.
    #[test]
    fn candidate_match_filter_survives_merge_with_request_min_should() {
        let key = JsonPath::new("tag");
        let candidates = HashSet::from([
            FacetValue::Keyword("red".to_string()),
            FacetValue::Int(7),
        ...
        let candidate_filter = candidate_match_filter(&key, &candidates);
        let request_filter = Filter::new_min_should(MinShould {
            conditions: vec![Condition::Field(FieldCondition::new_match(
                JsonPath::new("color"),
                Match::from("warm".to_string()),
            ))],
            min_count: 1,
        });
        let merged = candidate_filter.merge_owned(request_filter);

        let payloads: Vec<Payload> = vec![
            payload_json! {"tag": "red", "color": "warm"},
            payload_json! {"color": "warm"},
            payload_json! {"tag": "red"},
            payload_json! {"tag": 7, "color": "warm"},
        ];
        ...
        assert!(checker.check(0, &merged), "request + string candidate");
        assert!(
            !checker.check(1, &merged),
            "request condition alone is not enough"
        );
        assert!(
            !checker.check(2, &merged),
            "candidate alone is not enough"
        );
        assert!(checker.check(3, &merged), "request + integer candidate");
    }
}
use std::collections::{BTreeSet, HashMap, HashSet};
use std::sync::atomic::{AtomicBool, Ordering};
