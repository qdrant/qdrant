...
impl VectorStorage for MmapSparseVectorStorage {
    ...
    fn size_of_available_vectors_in_bytes(&self) -> usize {
        if self.metadata.next_point_offset == 0 {
            return 0;
        }
        let available_fraction = (self.metadata.next_point_offset - self.metadata.deleted_count)
            as f32
            / self.metadata.next_point_offset as f32;
        let available_size = (self.metadata.total_sparse_size as f32 * available_fraction) as usize;
