...
use crate::data_types::vectors::{DenseVector, QueryVector, Vector};
...
use crate::vector_storage::{
    RawScorer, VectorStorage as _, VectorStorageElementType, DEFAULT_STOPPED,
};

pub fn new<'a>(
    query: QueryVector,
    storage: &'a MemmapDenseVectorStorage<VectorStorageElementType>,
...
pub struct AsyncRawScorerImpl<'a, TQueryScorer: QueryScorer<[VectorStorageElementType]>> {
    points_count: PointOffsetType,
    query_scorer: TQueryScorer,
    storage: &'a MmapDenseVectors<VectorStorageElementType>,
    point_deleted: &'a BitSlice,
...
where
    TQueryScorer: QueryScorer<[VectorStorageElementType]>,
{
    fn new(
        points_count: PointOffsetType,
        query_scorer: TQueryScorer,
...
impl<'a> AsyncRawScorerBuilder<'a> {
    ...
    fn _build_with_metric<TMetric: Metric<VectorStorageElementType> + 'a>(
        self,
    ) -> OperationResult<Box<dyn RawScorer + 'a>> {
        let Self {
            points_count,
        ...
        match query {
            QueryVector::Nearest(vector) => {
                match vector {
                    Vector::Dense(dense_vector) => {
                        let query_scorer =
                            MetricQueryScorer::<VectorStorageElementType, TMetric, _>::new(
                                dense_vector,
                                storage,
                            );
            ...
            QueryVector::Recommend(reco_query) => {
                ...
                let query_scorer =
                    CustomQueryScorer::<VectorStorageElementType, TMetric, _, _, _>::new(
                        reco_query, storage,
                    );
                Ok(Box::new(AsyncRawScorerImpl::new(
                    points_count,
