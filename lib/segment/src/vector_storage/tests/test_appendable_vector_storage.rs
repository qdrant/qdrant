...
fn test_delete_points_in_appendable_memmap_vector_storages() {
    ...
    {
        let storage =
            open_appendable_memmap_vector_storage(dir.path(), 4, 4, Distance::Dot).unwrap();
        do_test_delete_points(storage.clone());
        storage.borrow().flusher()().unwrap();
    }

    let _storage = open_appendable_memmap_vector_storage(dir.path(), 4, 4, Distance::Dot).unwrap();
}
