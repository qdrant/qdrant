...
impl PayloadIndex for StructPayloadIndex {
    ...
    fn assign(&mut self, point_id: PointOffsetType, payload: &Payload, key: &Option<String>) -> OperationResult<()> {
        ...
        if let Some(key) = key {
            self.payload.borrow_mut().assign_by_key(point_id, payload, key)
        } else {
