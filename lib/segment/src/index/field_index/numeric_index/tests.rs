...
mod swap_tests {
    ...
    fn filter_ids(
        ...
        let mut result: Vec<_> = index
            .inner()
            .filter(
                &FieldCondition::new_range(JsonPath::new("unused"), ordered),
                &hw_counter,
            )
            .unwrap()
            .unwrap()
    ...
    #[test]
    fn immutable_to_mmap_and_back_preserves_files_and_results() {
        let (_temp_dir, mut index) = random_index(500, 2, IndexType::RamMmap);

        assert!(matches!(index.inner(), NumericIndexInner::Immutable(_)));
        ...
        let results_before = filter_ids(&index, query);
        assert!(!results_before.is_empty());

        // Immutable -> Mmap
        assert!(index.swap_on_disk(true).unwrap());
        assert!(matches!(index.inner(), NumericIndexInner::Mmap(_)));
        assert!(index.is_on_disk());
        assert_eq!(snapshot_files(&index), files_before);
        assert_eq!(filter_ids(&index, query), results_before);
    ...
    fn mutable_gridstore_reports_swap_not_applicable() {
        let (_temp_dir, mut index) = random_index(100, 1, IndexType::MutableGridstore);
        assert!(matches!(index.inner(), NumericIndexInner::Mutable(_)));
