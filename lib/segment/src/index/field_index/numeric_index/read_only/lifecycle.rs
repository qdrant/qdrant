    ...
    Vec<T>: Blob,
{
    ...
    /// [`ReadOnlyNumericIndexInner::open_gridstore`] and wraps the inner with
    ...
    pub fn open_gridstore(fs: &S::Fs, dir: PathBuf) -> OperationResult<Self> {
        Ok(Self {
            inner: ReadOnlyNumericIndexInner::open_gridstore(fs, dir)?,
...
// `open_mmap` is concrete to `S = MmapFile` for the same reason as on
// [`ReadOnlyNumericIndexInner::open_mmap`]: the immutable leaf
// [`UniversalNumericIndex::open`] is not yet filesystem-generic.
impl<T: Encodable + Numericable + StoredValue + Send + Sync + Default, P>
    ReadOnlyNumericIndex<T, P, MmapFile>
where
    Vec<T>: Blob,
{
    /// Read-only mirror of [`NumericIndex::new_mmap`][1]: forwards to
    /// [`ReadOnlyNumericIndexInner::open_mmap`] and wraps the inner with the
    /// typed payload-value phantom `P`. `Ok(None)` propagates from the inner
    /// when the on-disk index doesn't exist.
    ///
    ...
    pub fn open_mmap(
        ...
        Ok(
            ReadOnlyNumericIndexInner::open_mmap(path, is_on_disk, deleted_points)?.map(|inner| {
                Self {
                    inner,
                    _phantom: PhantomData,
                }
