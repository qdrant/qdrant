...
mod swap_tests {
    ...
    fn snapshot_files(index: &GeoMapIndex) -> BTreeMap<PathBuf, Vec<u8>> {
        GeoMapIndexRead::files(index)
            .into_iter()
            .filter(|p| p.exists())
            .map(|p| {
    ...
    #[test]
    fn geo_immutable_to_storage_round_trip_preserves_files() {
        let (mut index, _temp_dir, _db) = build_random_index(200, 4, IndexType::RamMmap);

        assert!(matches!(index, GeoMapIndex::Immutable(_)));
        let files_before = snapshot_files(&index);
        assert!(!files_before.is_empty());

        assert!(index.swap_on_disk(true).unwrap());
        assert!(matches!(index, GeoMapIndex::Storage(_)));
    ...
    fn mutable_gridstore_reports_swap_not_applicable() {
        let (mut index, _temp_dir, _db) = build_random_index(50, 2, IndexType::MutableGridstore);

        assert!(matches!(index, GeoMapIndex::Mutable(_)));
        assert!(!index.swap_on_disk(true).unwrap());
        assert!(matches!(index, GeoMapIndex::Mutable(_)));
    }
}
