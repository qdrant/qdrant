...
mod swap_tests {
    ...
    fn basic_config() -> TextIndexParams {
        TextIndexParams {
            r#type: TextIndexType::Text,
            tokenizer: TokenizerType::default(),
            min_token_len: None,
    ...
    fn snapshot_files(index: &FullTextIndex) -> BTreeMap<PathBuf, Vec<u8>> {
        index
            .files()
            .into_iter()
            .filter(|p| p.exists())
    ...
    #[test]
    fn full_text_immutable_to_mmap_round_trip_preserves_files() {
        let temp_dir = Builder::new().prefix("ft_swap").tempdir().unwrap();
        ...
        builder.init().unwrap();
        let hw_counter = HardwareCounterCell::new();
        for (idx, title) in movie_titles().into_iter().enumerate().take(40) {
            builder
                .add_many(idx as PointOffsetType, vec![title], &hw_counter)
                .unwrap();
        }
        ...
        assert!(matches!(index, FullTextIndex::Mmap(_)));
