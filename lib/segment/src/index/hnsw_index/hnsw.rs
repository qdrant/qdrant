...
impl<TGraphLinks: GraphLinks> HNSWIndex<TGraphLinks> {
    ...
    pub fn build_filtered_graph(
        ...
        let scorer_builder = vector_storage.scorer_builder();
            ...
            points_to_index
                ...
                .try_for_each(|block_point_id| {
                    ...
                    let raw_scorer = if let Some(quantized_raw_scorer) =
                        scorer_builder.quantized_raw_scorer(&vector)
                    {
                        quantized_raw_scorer
                    } else {
                        scorer_builder.raw_scorer(vector.to_owned())
                    };
    ...
    pub fn search_with_graph(
        ...
        let (raw_scorer, quantized) = if ignore_quantization {
            (scorer_builder.raw_scorer(vector.to_owned()), false)
        } else if let Some(quantized_raw_scorer) = scorer_builder.quantized_raw_scorer(vector) {
        ...
        if let Some(graph) = &self.graph {
            ...
            if quantized {
                let raw_scorer = scorer_builder.raw_scorer(vector.to_owned());
    ...
    fn search_vectors_plain(
        ...
        if ignore_quantization {
            vectors
                .iter()
                .map(|vector| {
                    vector_storage.scorer_builder().score_points(
                        vector,
            ...
            vectors
                .iter()
                .map(|vector| {
                    vector_storage.scorer_builder().score_quantized_points(
                        vector,
                        filtered_iter.as_mut(),
                        top,
                    )
...
impl<TGraphLinks: GraphLinks> VectorIndex for HNSWIndex<TGraphLinks> {
        ...
        match filter {
            None => {
                if exact {
                    ...
                    vectors
                        .iter()
                        .map(|vector| vector_storage.scorer_builder().score_all(vector, top))
                        .collect()
                } else {
    ...
    fn build_index(&mut self, stopped: &AtomicBool) -> OperationResult<()> {
        ...
        if self.config.m > 0 {
            let ids: Vec<_> = vector_storage.iter_ids().collect();
            ...
            pool.install(|| {
                ids.into_par_iter().try_for_each(|vector_id| {
                        ...
                        scorer_builder.quantized_raw_scorer(&vector)
                    {
                        ...
                        scorer_builder.raw_scorer(vector)
                    };
