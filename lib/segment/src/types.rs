...
impl From<&PayloadType> for PayloadInterface {
    fn from(payload: &PayloadType) -> Self {
        match payload {
            ...
            PayloadType::Geo(x) => {
                match x.len() {
                    1 => PayloadInterface::Payload(PayloadInterfaceStrict::Geo(PayloadVariant::Value(x.first().unwrap().clone()))),
                    _ => PayloadInterface::Payload(PayloadInterfaceStrict::Geo(PayloadVariant::List(x.clone())))
                }
            }
        }
    }
