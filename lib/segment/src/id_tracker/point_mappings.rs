...
#[derive(Debug)]
pub struct PointMappingsReadHolder<'a> {
    pub(crate) deleted: &'a BitSlice,
    mappings_state: RwLockReadGuard<'a, PointMappingsState>,
}

impl<'a> PartialEq for PointMappingsReadHolder<'a> {
    fn eq(&self, other: &Self) -> bool {
        self.deleted == other.deleted && *self.mappings_state == *other.mappings_state
    }
}
...
impl PointMappings {
    ...
    pub fn new(
        ...
        let mappings_state = PointMappingsState {
    ...
    pub fn deconstruct(
        ...
        let mappings_state = self.mappings_state.into_inner();
        (
            self.deleted,
            mappings_state.internal_to_external,
            mappings_state.external_to_internal_num,
            mappings_state.external_to_internal_uuid,
        )
    }
    ...
    pub(crate) fn set_link(
        ...
        let mappings_state = self.mappings_state.get_mut();
        let old_internal_id = match external_id {
            PointIdType::NumId(idx) => mappings_state
                .external_to_internal_num
                .insert(idx, internal_id),
            PointIdType::Uuid(uuid) => mappings_state
                .external_to_internal_uuid
                .insert(uuid, internal_id),
        };

        let internal_id = internal_id as usize;
        if internal_id >= mappings_state.internal_to_external.len() {
            mappings_state
                .internal_to_external
        ...
        mappings_state.internal_to_external[internal_id] = external_id;
    ...
    pub fn assert_mappings(&self) {
        let mappings_state = self.mappings_state.read();
        for (external_id, internal_id) in mappings_state.external_to_internal_num.iter() {
            debug_assert!(
                mappings_state.internal_to_external[*internal_id as usize]
    ...
    pub(crate) fn drop(&self, external_id: PointIdType) -> Option<PointOffsetType> {
        let mut mappings_state = self.mappings_state.write();
        let internal_id = match external_id {
            ...
            PointIdType::NumId(num) => mappings_state.external_to_internal_num.remove(&num),
            PointIdType::Uuid(uuid) => mappings_state.external_to_internal_uuid.remove(&uuid),
        };
    ...
    pub(crate) fn read(&self) -> PointMappingsReadHolder<'_> {
        PointMappingsReadHolder {
            deleted: &self.deleted,
            mappings_state: self.mappings_state.read(),
        }
    }
}

impl PointMappingsState {
...
impl<'a> PointMappingsReadHolder<'a> {
    ...
    #[inline]
    pub(crate) fn available_point_count(&self) -> usize {
        self.mappings_state.available_point_count()
    }

    pub(crate) fn internal_id(&self, external_id: &PointIdType) -> Option<PointOffsetType> {
        match external_id {
            PointIdType::NumId(num) => self
                .mappings_state
                .external_to_internal_num
                .get(num)
                .copied(),
            PointIdType::Uuid(uuid) => self
                .mappings_state
                .external_to_internal_uuid
                .get(uuid)
                .copied(),
    ...
    pub(crate) fn iter_random(
        ...
        let rng = rand::rng();
        let max_internal = self.mappings_state.internal_to_external.len();
        if max_internal == 0 {
        ...
        let iter = Distribution::sample_iter(uniform, rng)
            ...
            .filter_map(move |i| {
                if self.deleted[i] {
                    ...
                    Some((
                        self.mappings_state.internal_to_external[i],
    ...
    pub(crate) fn is_deleted_point(&self, key: PointOffsetType) -> bool {
        ...
        self.deleted.get(key).map(|x| *x).unwrap_or(true)
    }

    pub(crate) fn total_point_count(&self) -> usize {
        self.mappings_state.internal_to_external.len()
    }
}
