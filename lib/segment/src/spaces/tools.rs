...
pub fn peek_top_smallest_iterable<I, E: Ord>(elements: I, top: usize) -> OperationResult<Vec<E>>
where
    I: IntoIterator<Item = E>,
{
    ...
    Ok(pq.into_vec().into_iter().map(|Reverse(x)| x).collect())
}

pub fn peek_top_largest_iterable<I, E: Ord>(elements: I, top: usize) -> OperationResult<Vec<E>>
where
    I: IntoIterator<Item = E>,
{
    if top == 0 {
...
pub fn peek_top_scores<E: Ord + Clone>(scores: &[E], top: usize) -> OperationResult<Vec<E>> {
...
mod tests {
    ...
    fn test_peek_top() {
        ...
        let res = peek_top_scores(&data, 3).unwrap();
    ...
    fn test_peek_top_rev() {
        let data = vec![10, 20, 40, 5, 100, 33, 84, 65, 20, 43, 44, 42];
        let res = peek_top_smallest_iterable(data, 3).unwrap();
