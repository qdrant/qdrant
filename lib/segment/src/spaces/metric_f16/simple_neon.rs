...
#[cfg(target_feature = "neon")]
#[cfg(feature = "neon_fp16")]
pub(crate) unsafe fn euclid_similarity_neon_half(
...
#[cfg(target_feature = "neon")]
#[cfg(feature = "neon_fp16")]
pub(crate) unsafe fn manhattan_similarity_neon_half(
    v1: &[VectorElementTypeHalf],
    v2: &[VectorElementTypeHalf],
) -> ScoreType {
    let n = v1.len();
...
pub(crate) unsafe fn manhattan_similarity_neon_half(
    ...
    let v1_f32 = v1.iter().map(|x| f16::to_f32(*x)).collect_vec();
    let v2_f32 = v2.iter().map(|x| f16::to_f32(*x)).collect_vec();
    simple_neon::manhattan_similarity_neon(&v1_f32, &v2_f32)
}

#[cfg(target_feature = "neon")]
#[cfg(feature = "neon_fp16")]
pub(crate) unsafe fn dot_similarity_neon_half(
    v1: &[VectorElementTypeHalf],
    v2: &[VectorElementTypeHalf],
) -> ScoreType {
...
pub(crate) unsafe fn dot_similarity_neon_half(
    ...
    let v1_f32 = v1.iter().map(|x| f16::to_f32(*x)).collect_vec();
    let v2_f32 = v2.iter().map(|x| f16::to_f32(*x)).collect_vec();
    simple_neon::dot_similarity_neon(&v1_f32, &v2_f32)
}
...
mod tests {
    ...
    fn test_spaces_neon() {
        ...
        if std::arch::is_aarch64_feature_detected!("neon") {
            ...
            let euclid_simd = unsafe { euclid_similarity_neon_half(&v1, &v2) };
            let euclid = euclid_similarity_half(&v1, &v2);
            assert!((euclid_simd-euclid).abs()/euclid.abs() < 0.0005);
            

            let manhattan_simd = unsafe { manhattan_similarity_neon_half(&v1, &v2) };
