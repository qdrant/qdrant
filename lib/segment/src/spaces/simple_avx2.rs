...
#[target_feature(enable = "avx2")]
pub unsafe fn euclid_similarity_avx2(v1: &[VectorElementType], v2: &[VectorElementType]) -> ScoreType {
    let n = v1.len();
    let m = n - (n % 8);
...
#[target_feature(enable = "avx2")]
pub unsafe fn cosine_preprocess_avx2(vector: &[VectorElementType]) -> Vec<VectorElementType> {
    let n = vector.len();
    let m = n - (n % 8);
    let mut sum256: __m256 = _mm256_setzero_ps();
    for i in (0..m).step_by(8) {
...
#[target_feature(enable = "avx2")]
pub unsafe fn dot_similarity_avx2(v1: &[VectorElementType], v2: &[VectorElementType]) -> ScoreType {
    let n = v1.len();
    let m = n - (n % 8);
    let mut sum256: __m256 = _mm256_setzero_ps();
    for i in (0..m).step_by(8) {
