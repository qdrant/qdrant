...
fn test_filterable_hnsw() {
    ...
    for _i in 0..attempts {
        ...
        let index_result = hnsw_index.dissimilarity_search_with_graph(
            &query,
            filter_query,
            amount,
            Some(&SearchParams {
        ...
        let plain_amount = num_vectors as usize / 100; // 1% of all vectors
        let plain_result = segment.vector_data[DEFAULT_VECTOR_NAME]
            .vector_index
            .borrow()
            .dissimilarity_search(&[&query], filter_query, plain_amount, None, &false.into());

        for index in index_result {
            plain_result[0]
                .iter()
                .find(|x| x.idx == index.idx)
                .expect("index result should be present in plain result");
        }
    }
}
