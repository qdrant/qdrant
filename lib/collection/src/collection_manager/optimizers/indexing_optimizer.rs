...
mod tests {
    ...
    /// `execute_optimization` must hold `LockedSegmentHolder::acquire_updates_lock`
    /// across the proxy installation window so that a concurrent shard update
    /// (e.g. `CreateVectorName(V)` going through `CollectionUpdater::update`)
    /// cannot mutate the source segments between the optimizer capturing
    /// `target_config` and proxies replacing those segments in the holder.
    /// This test pins that invariant by asserting proxies are not yet
    /// installed while the updates lock is held. Note: `finish_optimization`
    /// also takes the same lock, so we cannot just observe "optimization
    /// completed" — without the proxy-install guard, the worker would race
    /// past proxy install and only block later inside `finish_optimization`.
    #[test]
    fn test_optimize_blocks_proxy_install_on_updates_lock() {
        use std::sync::Arc;
        ...
        let mut holder = SegmentHolder::default();
        let seg1 = random_segment(segments_dir.path(), opnum.next().unwrap(), 25, dim);
        let seg2 = random_segment(segments_dir.path(), opnum.next().unwrap(), 25, dim);
        let id1 = holder.add_new(seg1);
        let id2 = holder.add_new(seg2);
        ...
        // Force optimization to merge both segments.
        index_optimizer
            .threshold_config_mut_for_test()
        ...
        // Hold the updates lock — emulates the path `CollectionUpdater::update`
        // takes when applying a `CreateVectorName` operation.
        let updates_guard = locked_holder.acquire_updates_lock();

        let optimize_done = Arc::new(AtomicBool::new(false));
        let optimize_done_clone = Arc::clone(&optimize_done);
        let holder_clone = locked_holder.clone();
        let optimize_thread = std::thread::spawn(move || {
            index_optimizer.optimize_for_test(holder_clone, vec![id1, id2]);
            optimize_done_clone.store(true, Ordering::SeqCst);
        });

        // Give the worker time to enter `execute_optimization` and reach the
        // proxy-install region. Without the fix, it would race past install
        // before we drop the guard.
        std::thread::sleep(Duration::from_millis(250));
        assert!(
            !optimize_done.load(Ordering::SeqCst),
            "optimize completed while the updates lock was held",
        );

        // The actual fix invariant: while the updates lock is held, no
        // segment in the holder has been replaced with a proxy yet — that is,
        // the worker is blocked *before* proxy install, not just before
        // `finish_optimization` (which would already block on the same lock
        // without the fix).
        let segments_are_originals = locked_holder
            .read()
            .iter()
            .all(|(_, s)| matches!(s, LockedSegment::Original(_)));
        assert!(
            segments_are_originals,
            "execute_optimization installed proxies while the updates lock was held — \
             proxy install is not gated by the lock, so a concurrent shard update can \
             still mutate source segments mid-optimization",
        );

        drop(updates_guard);
        optimize_thread.join().unwrap();
        assert!(optimize_done.load(Ordering::SeqCst));
    }
}
