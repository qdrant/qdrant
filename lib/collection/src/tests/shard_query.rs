...
use segment::types::{PointIdType, WithPayload, WithPayloadInterface, WithVector};
...
async fn test_shard_query_payload_vector() {
    ...
    let query = ShardQueryRequest {
        ...
        with_payload: WithPayload::from(WithPayloadInterface::Bool(true)), // requesting payload
    };
