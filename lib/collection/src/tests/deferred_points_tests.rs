...
async fn count_visible(shard: &LocalShard) -> u64 {
    let mut visible = 0;
    for i in 1..=NUM_POINTS {
        if retrieve_point(shard, i).await {
            visible += 1;
...
/// A shard that is started while already holding a deferred backlog must make that
/// backlog visible on its own. Nothing writes to it after it comes up, which is the
/// state a replica is left in when it receives a shard by transfer or resharding and
/// then sees no further traffic.
///
/// Phase 2 reopens with optimization enabled and sends nothing. Recovery replays the
/// WAL synchronously, which bypasses the update worker, so the optimizer is never
/// signalled and the backlog stays hidden.
#[tokio::test(flavor = "multi_thread")]
async fn test_deferred_points_become_visible_without_further_writes() {
    let _ = env_logger::builder().is_test(true).try_init();

    let collection_dir = Builder::new()
        .prefix("test_deferred_activation")
        .tempdir()
        .unwrap();
    let payload_schema_dir = Builder::new().prefix("qdrant-test").tempdir().unwrap();
    let payload_index_schema = Arc::new(
    ...
    let shard = build_shard(&config, collection_dir.path(), payload_index_schema.clone()).await;
    let hw_acc = HwMeasurementAcc::new();
    for i in 1..=NUM_POINTS {
        shard
            .update(
            ...
            .await
            .unwrap_or_else(|err| panic!("upsert of point {i} must succeed: {err}"));
    }

    let deferred_before = NUM_POINTS - count_visible(&shard).await;
    assert!(
        deferred_before > 0,
        "setup: with optimization disabled some points must stay deferred, \
         otherwise there is no backlog to inherit",
    );
    shard.stop_gracefully().await;

    config.optimizer_config.max_optimization_threads = Some(2);
    let shard = load_shard(&config, collection_dir.path(), payload_index_schema).await;

    let deadline = tokio::time::Instant::now() + Duration::from_secs(30);
    let mut visible = count_visible(&shard).await;
    while visible < NUM_POINTS && tokio::time::Instant::now() < deadline {
        tokio::time::sleep(Duration::from_millis(200)).await;
        visible = count_visible(&shard).await;
    }
    ...
    assert_eq!(
        visible,
        NUM_POINTS,
        "a shard started with {deferred_before} deferred points must optimize them into \
         visibility on its own; {} are still hidden after 30s, so the backlog is only ever \
         cleared by an incoming write",
        NUM_POINTS - visible,
    );
}
