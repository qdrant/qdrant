...
fn check_validation_error(result: Result<(), ::validator::ValidationErrors>) {
    println!("{:?}", result);
    match result {
...
fn validate_error_sparse_vector_point_struct() {
    check_validation_error(wrong_point_struct().validate());
}
...
fn validate_error_sparse_vector_query_enum() {
    check_validation_error(QueryEnum::Nearest(wrong_named_vector_struct()).validate());
}
...
fn validate_error_sparse_vector_recommend_example() {
    check_validation_error(RecommendExample::Sparse(wrong_sparse_vector()).validate());
}
