...
impl Collection {
    ...
    /// Restart a shard transfer with a new method, updating the transfer record
    /// **in place** instead of composing an inner Abort followed by an inner
    /// Start (finding E).
    ///
    /// `transfer_key` identifies the existing record; `new_transfer` is the
    /// replacement (`to_shard_id: None`, the new method, and the old record's
    /// `sync` preserved — byte-matching the old Abort∘Start result).
    ///
    /// Replay walk (invariant 1): the record's method update
    /// (`register_restart_transfer`, step 6) is the LAST durable write, so it
    /// gates the operation, and the record is never removed at any point. Crash
    /// before step 6 → the record still carries the old method → the gate below
    /// proceeds and re-runs steps 2–6, all idempotent (`stop_task` no-ops if the
    /// task is gone, `revert_proxy_shard_to_local` is runtime-only,
    /// `init_empty_local_shard` re-wiping an empty shard is a no-op in effect,
    /// the replica-state write is absolute). Crash after step 6 → the record
    /// carries the new method → the gate returns an idempotent no-op. Because the
    /// record is present throughout, neither of finding E's windows (no transfer
    /// found; method unchanged) can dismiss the entry with partial state.
        ...
        F: Future<Output = ()> + Send + 'static,
    {
        ...
        // 1. Gate. The record is never removed mid-restart, so:
        //    - absent  → stale-duplicate restart → dismiss (bad_request).
        //    - method already new → replay whose final write landed → no-op.
        {
        ...
        // 2. Stop the running transfer task for the *old* record (runtime-only).
        let _ = self
            .transfer_tasks
            .lock()
            .await
...

        {
            ...
            //    any partially-transferred data. Safe because a restart is only
            //    ever proposed as a WAL-delta-transfer fallback (never resharding),
            //    and the fallback method (snapshot/stream) fully re-populates the
            //    destination, so starting from empty converges to the same data.
            //    Replaying this (re-wiping an already-empty shard) is a no-op in
            //    effect.
            if is_receiver {
            ...
            //    initial state (absolute write).
            dest_replica_set
                .ensure_replica_with_state(new_transfer.to, initial_state)
                .await?;

            // 6. LAST durable write: update the record's method in place. Gates
            //    the whole operation on replay.
