...
pub async fn drop_all_shards(collection_path: &Path, shard_id: ShardId) -> CollectionResult<()> {
    for (_version, path) in shards_versions(collection_path, shard_id).await? {
        // delete old shard's data folder
