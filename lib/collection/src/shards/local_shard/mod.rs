...
impl LocalShardClocks {
    ...
    pub async fn copy_data(from: &Path, to: &Path) -> CollectionResult<()> {
        let highest_path_from = Self::newest_observed_clock_map_path(from);
        let cutoff_clock_map_path = Self::oldest_resolvable_clock_map_path(from);

        if highest_path_from.exists() {
            let highest_path_to = Self::newest_observed_clock_map_path(to);
            copy(highest_path_from, highest_path_to).await?;
        }

        if cutoff_clock_map_path.exists() {
            let cutoff_path_to = Self::oldest_resolvable_clock_map_path(to);
    ...
    pub async fn move_data(from: &Path, to: &Path) -> CollectionResult<()> {
        let highest_path_from = Self::newest_observed_clock_map_path(from);
        let cutoff_path_from = Self::oldest_resolvable_clock_map_path(from);
    ...
    pub async fn delete_data(shard_path: &Path) -> CollectionResult<()> {
        let highest_path = Self::newest_observed_clock_map_path(shard_path);
        let cutoff_path = Self::oldest_resolvable_clock_map_path(shard_path);
