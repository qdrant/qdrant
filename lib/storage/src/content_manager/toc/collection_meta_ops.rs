    ...
    async fn handle_transfer(
        ...
        match transfer_operation {
            ...
            ShardTransferOperations::Restart(transfer_restart) => {
                ...
                // The record must exist. Because a restart now updates the record
                // in place (never removing it), absence means a stale-duplicate
                // restart — dismiss it. A restart to an *unchanged* method is no
                // longer rejected here; it is an idempotent no-op inside
                // `restart_shard_transfer` (finding E's replay window).
                ...
                // to_shard_id and any filter, switch to the new method (byte-matches
                // the old Abort∘Start result).
