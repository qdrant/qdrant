...
pub async fn do_delete_full_snapshot(
    ...
    let task = tokio::spawn(async move {
        let snapshot_dir = get_full_snapshot_path(dispatcher.toc(), &snapshot_name)
            .await
            .unwrap();
...
pub async fn do_delete_collection_snapshot(
    ...
    let task = tokio::spawn(async move {
        ...
        let file_name = collection.get_snapshot_path(&snapshot_name).await.unwrap();
...
pub async fn do_create_full_snapshot(
    ...
    let task = tokio::spawn(async move {
        ...
        for collection_name in &all_collections {
            let snapshot_details = dispatcher.create_snapshot(collection_name).await.unwrap();
        ...
        get_snapshot_description(&full_snapshot_path).await.unwrap()
    });
