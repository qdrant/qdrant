...
pub unsafe fn avx_cosine_similarity_bytes(
    v1: &[u8],
    v2: &[u8],
) -> f32 {
    ...
    for _ in 0..len / 32 {
        ...
        let mul16 = _mm256_mullo_epi16(masked_p1, masked_p1);

        norm1 = _mm256_add_epi32(norm1, _mm256_and_si256(mul16, mask_epu32_epu16));
        let mul16 = _mm256_bsrli_epi128(mul16, 2);
        norm1 = _mm256_add_epi32(norm1, _mm256_and_si256(mul16, mask_epu32_epu16));

        let masked_p2 = _mm256_and_si256(p2, mask_epu16_epu8);
        let mul16 = _mm256_mullo_epi16(masked_p2, masked_p2);

        norm2 = _mm256_add_epi32(norm2, _mm256_and_si256(mul16, mask_epu32_epu16));
        let mul16 = _mm256_bsrli_epi128(mul16, 2);
        norm2 = _mm256_add_epi32(norm2, _mm256_and_si256(mul16, mask_epu32_epu16));

        let mul16 = _mm256_mullo_epi16(masked_p1, masked_p2);
