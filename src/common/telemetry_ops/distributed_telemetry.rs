...
pub struct DistributedTelemetry {
    ...
    cluster: DistributedClusterTelemetry,
}
...
impl DistributedTelemetry {
    pub fn resolve_telemetries(
        ...
        // Build peers map from leader's cluster info
        let base_cluster = base_telemetry.cluster.as_ref().ok_or_else(|| {
            StorageError::service_error("TelemetryData is missing `cluster` field")
        })?;
        let number_of_peers = base_cluster
            .status
            .as_ref()
            .map(|s| s.number_of_peers as u64);
        ...
        // Aggregate peers info
        let peers = aggregate_peers_info(missing_peers, base_cluster, telemetry_by_peer)
            .unwrap_or_default();

        let cluster = DistributedClusterTelemetry {
