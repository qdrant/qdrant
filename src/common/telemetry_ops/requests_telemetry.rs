...
pub struct ActixTelemetryCollector {
    ...
    pub per_collection_metrics_enabled: bool,
}
...
pub struct ActixWorkerTelemetryCollector {
    ...
    methods_per_collection: HashMap<
        String,
        HashMap<String, HashMap<HttpStatusCode, Arc<Mutex<OperationDurationsAggregator>>>>,
    >,
    /// Whether per-collection metrics tracking is enabled
    per_collection_metrics_enabled: bool,
}
...
impl ActixTelemetryCollector {
    pub fn create_web_worker_telemetry(&mut self) -> Arc<Mutex<ActixWorkerTelemetryCollector>> {
        let worker: Arc<Mutex<_>> = Arc::new(Mutex::new(ActixWorkerTelemetryCollector {
            methods: HashMap::new(),
            methods_per_collection: HashMap::new(),
            per_collection_metrics_enabled: self.per_collection_metrics_enabled,
        }));
...
impl TonicWorkerTelemetryCollector {
    pub fn add_response(
        ...
        // Add to per-collection statistics only if enabled and collection is provided
        if self.per_collection_metrics_enabled {
...
impl Anonymize for WebApiTelemetry {
    fn anonymize(&self) -> Self {
        ...
        let mut aggregated_per_collection: HashMap<
            String,
        ...
        WebApiTelemetry {
            responses,
            responses_per_collection: HashMap::new(),
        }
    }
}
...
impl Anonymize for GrpcTelemetry {
    fn anonymize(&self) -> Self {
        ...
        let mut aggregated_per_collection: HashMap<
            String,
            HashMap<GrpcStatusCode, OperationDurationStatistics>,
        > = HashMap::new();
        for (method, collections) in &self.responses_per_collection {
            let method_entry = aggregated_per_collection.entry(method.clone()).or_default();
            for (_collection, status_codes) in collections {
