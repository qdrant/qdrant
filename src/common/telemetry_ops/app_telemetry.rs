...
pub struct RunningEnvironmentTelemetry {
    ...
    is_docker: bool,
    /// Container runtime Qdrant runs under, or `none` for bare metal / VM.
    /// Unlike `is_docker` (Docker-only), this also detects Kubernetes, Podman, etc.
    #[anonymize(false)]
...
/// Container runtime Qdrant is running under, detected from well-known markers.
/// Best-effort: a runtime that leaves no recognizable marker reports `Other`
/// (overlay root) or, failing that, `None` (assumed bare metal / VM).
#[derive(Serialize, Clone, Copy, Debug, JsonSchema, Anonymize)]
#[serde(rename_all = "snake_case")]
...
impl ContainerRuntime {
    /// Detect the runtime, most specific marker first. Non-Linux hosts match no
    /// marker and report `None`.
    pub fn detect() -> Self {
        // Kubernetes injects this into every pod; containerd/CRI-O leave no
        // `/.dockerenv`, so this is the only reliable signal there.
        ...
        // Any other container marker — Podman/Buildah (`/run/.containerenv`),
        // LXC/systemd-nspawn (`/run/systemd/container`), or an overlay root fs
        // (a container image that left no specific marker).
...
/// Whether `/` is an overlay filesystem (typical of container images).
