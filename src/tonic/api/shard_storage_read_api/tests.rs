...
async fn create_service_async() -> (ShardStorageReadService<MmapFile>, TempDir, PathBuf) {
    tokio::task::spawn_blocking(create_service).await.unwrap()
}

async fn drop_service(service: ShardStorageReadService<MmapFile>, storage_dir: TempDir) {
...
fn create_service() -> (ShardStorageReadService<MmapFile>, TempDir, PathBuf) {
    ...
    let service = ShardStorageReadService::new(toc);
...
async fn file_exists_rejects_path_traversal() {
    ...
    let err = service
        .file_exists(Request::new(ShardFileExistsRequest {
...
async fn list_files_returns_paths_relative_to_shard_dir() {
    ...
    let mut paths = service
        .list_files(Request::new(ShardListFilesRequest {
            collection_name: TEST_COLLECTION_NAME.to_string(),
            shard_id: TEST_SHARD_ID,
...
async fn file_length_returns_file_size() {
    ...
    let response = service
        .file_length(Request::new(ShardFileLengthRequest {
            collection_name: TEST_COLLECTION_NAME.to_string(),
            shard_id: TEST_SHARD_ID,
...
async fn read_bytes_returns_requested_range() {
    ...
    let response = service
        .read_bytes(Request::new(ShardReadBytesRequest {
            collection_name: TEST_COLLECTION_NAME.to_string(),
            shard_id: TEST_SHARD_ID,
...
async fn read_bytes_stream_splits_large_reads_into_chunks() {
    ...
    let mut stream = service
        .read_bytes_stream(Request::new(ShardReadBytesStreamRequest {
            collection_name: TEST_COLLECTION_NAME.to_string(),
            shard_id: TEST_SHARD_ID,
...
async fn read_whole_returns_entire_file() {
    ...
    let response = service
        .read_whole(Request::new(ShardReadWholeRequest {
            collection_name: TEST_COLLECTION_NAME.to_string(),
            shard_id: TEST_SHARD_ID,
...
async fn read_batch_returns_each_requested_slice() {
    ...
    let response = service
        .read_batch(Request::new(ShardReadBatchRequest {
            collection_name: TEST_COLLECTION_NAME.to_string(),
            shard_id: TEST_SHARD_ID,
...
async fn read_multi_reads_ranges_in_request_order() {
    ...
    let response = service
        .read_multi(Request::new(ShardReadMultiRequest {
            collection_name: TEST_COLLECTION_NAME.to_string(),
            shard_id: TEST_SHARD_ID,
            reads: vec![
                ReadMultiEntry {
